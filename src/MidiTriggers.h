...
public:
    ...
    ofEvent<void>& GetTimeResetEvent() { return mTimeResetEvent; }
    ofEvent<const int>& GetTickEvent() { return mTickEvent; }
    ...
    ofEvent<const NoteData&>& GetNoteOnEvent(int channel) { return mNoteOnEvents[channel]; }

private:
    void newMidiMessage(ofxMidiMessage& message) override;

    void RaiseTimeReset() { mTimeResetEvent.notify(this); }
    void RaiseTick() { mTickEvent.notify(this, mTickCount); }
    ...
    ofEvent<const NoteData&> mNoteOnEvents[16];
};
